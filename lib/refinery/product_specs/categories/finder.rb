    ...
    module Categories
      class Finder
        ...
        def with_globalize
          ...
          Refinery::ProductSpecs::Category.where(globalized_conditions).
               joins(:translations).
        ...
        def translated_attributes
          Refinery::ProductSpecs::Category.translated_attribute_names.map(&:to_s) | %w(locale)
        end
        ...
        def translations_conditions(original_conditions)
          ...
          original_conditions.keys.each do |key|
            if translated_attributes.include? key.to_s
              translations_conditions["#{Refinery::ProductSpecs::Category.translation_class.table_name}.#{key}"] = original_conditions.delete(key)
            end
          end
          translations_conditions
        end
      ...
      class FinderByScopedPath < FinderByPath
        ...
        def parent_category
          ...
          if parent_category_segment.friendly_id?
            ...
            Refinery::ProductSpecs::Category.find(parent_category_segment)
          end
        end
        ...
        def find
          if path.present?
            if path.friendly_id?
              ...
              Refinery::ProductSpecs::Category.friendly.find(path)
            end
          elsif id.present?
            Refinery::ProductSpecs::Category.friendly.find(id)
          end
        end
